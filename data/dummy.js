
const categories = [
    {
        title: 'Kitchen Items',
        image: '/site/kitchenwares.png',
    },
    {
        title: 'Souviners',
        image: '/site/souviners.png',
    },
    {
        title: 'Bedsheets and Duvets',
        image: '/site/landing-bedspread.png',
    },
    {
        title: 'Bathroom Items',
        image: '/site/landing-basket.png',
    },
    {
        title: 'Car Accessories',
        image: '/site/phone-holder.png',
    },
    {
        title: 'Others',
        image: '/site/others.png',
    }
] 

const products = [
    {   id: 1,
        title: 'Hundreds of Household Items',
        subText: 'Beautiful Artifacts',
        image: '/site/landing-pic1.png',
        buttonText: 'Shop Now'
    },
    {   id: 2,
        title: 'Hundreds of Household Items',
        subText: 'Beautiful Artifacts',
        image: '/site/landing-bedspread.png',
        buttonText: 'Shop Now'
    },
    {   id: 3,
        title: 'Hundreds of Household Items',
        subText: 'Beautiful Artifacts',
        image: '/site/landing-bedspread2.png',
        buttonText: 'Shop Now'
    }
]

export const productInfoData = [
    // {
    //     type: 'Introducing',
    //     title: 'Effective Souvenir for Weddings',
    //     subText: 'Lorem ipsum dolor sit amet, incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.',
    //     image: '/videos/spinningmop.mp4',
    //     buttonText: 'Learn More'
    // },
    {
        type: 'Introducing',
        title: 'Quality laundry basket',
        subText: 'High qulity and affordable laundry baskets at just N15,500. We have beautiful ash and white colors available . Size - 70 litre',
        image: '/videos/baskets.mp4',
        buttonText: 'Product Page'
    }
] 


export const recommendedProducts = [
    {   id: 7,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/colored-plates.png',
        images: ['/products/colored-plates.png']
    },
    {   id: 9,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/towel-colored.png',
        images: ['/products/towel-colored.png'],
    },
    {   id: 8,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/hose.png',
        images: ['/products/hose.png', '/products/towel-colored.png', '/products/towel-white.png'],
    },
    {   id: 10,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/towel-white.png',
        images: ['/products/towel-white.png', '/products/cuttingboard.png'],
    },
    {   id: 1,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/cuttingboard.png',
        images: ['/products/cuttingboard.png'],
    },
    {   id: 0,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/coffee-cup.png',
        images: ['/products/coffee-cup.png'],
    },
    {   id: 2,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/knives.png',
        images: ['/products/knives.png'],
    },
    {   id: 3,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/pot.png',
        images: ['/products/pot.png'],
    },
    {   id: 6,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/pot.png',
        images: ['/products/pot.png'],
    },
    {   id: 5,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/colored-plates.png',
        images: ['/products/colored-plates.png'],
    },
    {   id: 4,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/squeezer.png',
        images: ['/products/squeezer.png'],
    },
    {   id: 11,
        title: 'consectetur adipiscing elit',
        price: '9,000',
        image: '/products/hose.png',
        images: ['/products/hose.png'],
    }
] 

export { categories, products }